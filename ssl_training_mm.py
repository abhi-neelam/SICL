...
def parse_arguments():
    ...
    parser.add_argument('--loss', default='super_subject')
    parser.add_argument('--run_seeds', action='store_true', default=False)
...
def ssl_pre_training(args, modalities, experiment_cfg, ssl_cfg, dataset_cfg, model_cfgs, transform_cfgs, augmentation_cfgs, experiment_id, loggers_list, loggers_dict):
    ...
    if args.framework == 'cmc':
        model = ContrastiveMultiviewCoding( args, modalities, encoders, **ssl_cfg['kwargs'])
    elif args.framework == 'cmc-cmkm':
        similarity_metrics = init_similarity_metrics(args, modalities, ssl_cfg, dataset_cfg)
        model = ContrastiveMultiviewCodingCVKM(args, modalities, encoders, similarity_metrics, **ssl_cfg['kwargs'])
...
def init_similarity_metrics(args, modalities, ssl_cfg, dataset_cfg):
    ...
    SUPPORTED_MODALITIES = ["inertial", "skeleton", "emg", "insole"]
    ...
    if cmkm_config['similarity_metric'] == "pretrained_encoder":
        for i, m in enumerate(modalities):
            ...
            pretrained_model = init_ssl_pretrained_original(pretrained_encoder_cfg, args.cmkm_pretrained_encoder_paths[i])
...
def main():
    ...
    print("tau: ", args.tau)
    print("experiment", args.experiment_config_path)
    print("augmentation", args.augmentations_path)
