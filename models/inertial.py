...
class SupervisedCNN1D(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        x = batch['inertial']
        if self.training_label == 'label':
            y = batch['label'] - 1
        elif self.training_label == 'label2':
            y = batch['label2'] - 1
        elif self.training_label == 'subject':
            y = batch['subject'] - 1 
        elif self.training_label == 'scene':
            y = batch['scene'] - 1
        else :
            raise ValueError(f"Training label {self.training_label} not supported")
        out = self(x)
