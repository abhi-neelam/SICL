...
class l2Manager:
    ...
    def get_table(self):
        ...
        for file_path in all_files:
            tmp_instance = l2Instance(file_path)
            modality = tmp_instance.parse_modality().lower() 
            ext = tmp_instance.parse_extension()
            if np.nan not in [modality, ext, tmp_instance.label, tmp_instance.subject, tmp_instance.trial, tmp_instance.label2] and ext in DATA_EXTENSIONS:
                out_table.append((file_path, modality, ext, tmp_instance.label, tmp_instance.subject, tmp_instance.trial, tmp_instance.label2))  # Added label2
        return pd.DataFrame(out_table, columns=['path', 'modality', 'extension', 'label', 'subject', 'trial', 'label2']) # Added label2
...
class l2Instance:
    def __init__(self, file_):
        self._file = file_
        self.label, self.subject, self.trial, self.label2= self.parse_subject_label()
    def parse_subject_label(self):
        filename = os.path.split(self._file)[1]
        try:
            return [int(file_[1:]) for file_ in filename.split('_')[:3]]+[int( filename.split('_')[3])] # Added label2
        except ValueError:
...
class UTDInertialInstance(l2Instance):
    def __init__(self, file_):
...
class UTDSkeletonInstance(l2Instance):
    def __init__(self, file_):
