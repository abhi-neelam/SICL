...
from models.simclr_um import SimCLRUnimodal , SimCLRUnimodal_original
...
def init_ssl_pretrained(args,model_cfg, ckpt_path):             # TODO changed correct later
    encoder = init_ssl_encoder(model_cfg)
    class_ = SimCLRUnimodal(args,'inertial', encoder, encoder.out_size)
    return class_.load_from_checkpoint(    ckpt_path,
    args=args,
    modality='inertial',
    encoder=encoder,
    mlp_in_size=encoder.out_size,
    strict=False
    )
    #return class_.load_from_checkpoint(ckpt_path, encoder=encoder, mlp_in_size=encoder.out_size, strict=False)


def init_ssl_pretrained_original(model_cfg, ckpt_path):
    encoder = init_ssl_encoder(model_cfg)
    class_ = SimCLRUnimodal_original('inertial', encoder, encoder.out_size)
    return class_.load_from_checkpoint(ckpt_path, encoder=encoder, mlp_in_size=encoder.out_size, strict=False)
